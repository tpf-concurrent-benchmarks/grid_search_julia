...
function unfold(self::Work, precision::Integer = -1)
    # values::Vector{Vector{Float64}} = Vector{NTuple{Float64}}(undef, self.size)
    ...
    for pos in 2:self.size
        ...
        for (i, curr_val) in enumerate(values[pos - 1])
            ...
            if curr_val + step < _end
                ...
                # values[pos] = (values[pos][1:i-1]..., updated, values[pos][i+1:@INTERVALS]...)
                values[pos, i] = updated
                break
            else
...
#=
function evaluate_for(f::Function, self::Work)
